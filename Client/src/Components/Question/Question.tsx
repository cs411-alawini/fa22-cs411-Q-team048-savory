...
export default function Question(props: NaviagtionState){
    ...
    const newItems = _copyAndSort(questions, currColumn.fieldName!, currColumn.isSortedDescending);
    ...
    const cols: IColumn[] = [
        {
          ...
          onRender: (item: IQuestion) => {
            return <a style={{color: "#0064BF"}} href="javascript:void(0)" onClick={() => {props.setNavState('-1'); props.setNavQuestionId!(Number(item.id));}}>{item.question}</a>;
          },
          data: 'string',
          isPadded: true,
        },
